...
## se encontrar face ######################################################
          ...
          cv2.putText(img,posiçãoX,(10,10), cv2.FONT_HERSHEY_PLAIN, 1.0,azul)
          posiçãoY = controlaposicao.controlaposicaoY(y)
          cv2.putText(img,posiçãoY,(10,20), cv2.FONT_HERSHEY_PLAIN, 1.0,azul)
          if(contole == "nao_encontrado"):
              comunica.escrever_arduino('6')#brilla os olhos rapidament 
              #arduino.enviar('6')#brilla os olhos rapidament       
              contole = "encontrado"          
###############################################################################
            
 ############ se nao encontar faces ############################################
          if(x == "vazio"):   
              cv2.putText(img,"nao encontrado",(10,10), cv2.FONT_HERSHEY_PLAIN, 1.0,vermelho)
              cv2.putText(img,"procuranco",(10,20), cv2.FONT_HERSHEY_PLAIN, 1.0,vermelho)
              #print(contole)
